...
**Balance is cached, not computed on read.** In a real DB-backed system this problem doesn't arise — balance would either be a column updated atomically within a database transaction alongside the insert, or derived via a SQL aggregate (`SELECT SUM(amount) ... GROUP BY type`) where the DB handles both atomicity and performance. Neither option is available here. With an in-memory store, the only choices are O(n) — sum all transactions on every read — or O(1) — cache the running balance. The cached approach was chosen, which means the service must compute the new balance and pass it down to the repository on every write. This is a consequence of the in-memory constraint, not a general pattern.
